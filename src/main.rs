...
fn parse(string: impl AsRef<OsStr>) -> Pattern {
...
fn parse_component(component: std::path::Component<'_>) -> PatternComponent {
    match component {
...
fn parse_tokens(mut string: &[u8], mut cond: impl FnMut(&[u8]) -> bool) -> (Tokens, &[u8]) {
    let mut tokens = vec![];
...
fn next_token(string: &[u8]) -> (Token, &[u8]) {
...
type TokenResult<'a> = Result<(Token, &'a [u8]), &'a [u8]>;

fn token_any_character(string: &[u8]) -> TokenResult {
...
fn token_wildcard(string: &[u8]) -> TokenResult {
    if string.get(0) == Some(&b'*') {
...
fn token_alternatives(mut string: &[u8]) -> TokenResult {
...
fn token_character_class(mut string: &[u8]) -> TokenResult {
    let original_string = string;
    if string.get(0) == Some(&b'[') {
        string = &string[1..];
        let mut classes = vec![];
...
fn token_repeat(mut string: &[u8]) -> TokenResult {
    let original_string = string;
    if string.get(0) == Some(&b'<') {
        string = &string[1..];
...
fn token_literal_string(string: &[u8]) -> TokenResult {
...
fn main() {
    ...
    let result = parse(pattern_string);
